...
var MY_LEARNING_VERSION = '1.0.81'; // dynamically set in distribution files
...
window.MyLearning = {
  ...
  // topicId: string,
  // chapterId: string,
  pageType: typeof window.W3S_PAGE_TYPE !== 'undefined' ? window.W3S_PAGE_TYPE : null,
  // lessonId: string,
  ...
  loadSharedApiModuleForAuthenticatedUser: null,
  _userStateFetchInFlight: null,
...
/*
  ...
  - Bottom status bar
*/
MyLearning.extraFeaturesEnabled = function (params) {
  try {
    if (typeof params === 'undefined') {
      params = {};
    }
    if (!UserSession.userHasExtraFeaturesAccess()) {
      MyLearning._logDebug('ExtraFeaturesEnabled -> false: user is not paying (not logged in or on free/closed plan without full access)');
      return false;
    }
    if (!params.skipMainElementCheck) {
      var main = document.getElementById('main');
      if (!main) {
        MyLearning._logDebug('ExtraFeaturesEnabled -> false: main element not found');
        return false;
      }
    }
    ...
    // Handle quiz pages - use folder from W3S_QUIZ if available
    if (MyLearning.pageType && MyLearning.pageType === 'quiz' && typeof window.W3S_QUIZ !== 'undefined' && window.W3S_QUIZ.folder) {
      path = window.W3S_QUIZ.folder;
    }

    // Disable on certain paths
    var skipPaths = ['/', '/index.php', '/index.html', '/default.asp', '/where_to_start.asp', '/tryit/default.asp', '/bootstrap/bootstrap_ver.asp',
    // below are any paths that start with:
    '/academy/', '/bootcamp/', '/training/', '/spaces/', '/league/', '/videos/', '/typingspeed/', '/forum/', '/leaderboard/', '/newsletter/', '/pathfinder/', '/plus/', '/signup/'];

    // Check for exact matches
    if (skipPaths.indexOf(path) !== -1) {
      MyLearning._logDebug('ExtraFeaturesEnabled -> false: path is in skipPaths', {
        path: path
      });
      return false;
    }

    // Check if path starts with any skip path that ends with '/' (but not '/')
    for (var i = 0; i < skipPaths.length; i++) {
      if (skipPaths[i] !== '/' && skipPaths[i].endsWith('/') && path.startsWith(skipPaths[i])) {
        MyLearning._logDebug('ExtraFeaturesEnabled -> false: path starts with skipPath', {
          path: path,
          skipPath: skipPaths[i]
        });
        return false;
      }
    }

    // Extract path parts and check if we have a meaningful path
    var pathParts = path.split('/').filter(function (part) {
      return part.length > 0;
    });
    if (pathParts.length === 0) {
      MyLearning._logDebug('ExtraFeaturesEnabled -> false: no meaningful path parts', {
        path: path
      });
      return false;
    }

    // Check if path matches a recognized topic
    var pathTopic = pathParts[0].toLowerCase();
    // Handle special cases
    if (pathTopic === 'javascript') pathTopic = 'js';
    if (pathTopic === 'csharp') pathTopic = 'cs';

    // Topic map - recognized tutorial topics
    var topicMap = {
      accessibility: true,
      angular: true,
      angularjs: true,
    ...
    if (MyLearning.pageType && ['page', 'lesson'].indexOf(MyLearning.pageType) === 0 && !topicMap[pathTopic]) {
      MyLearning._logDebug('ExtraFeaturesEnabled -> false: not a known topic for page/lesson type', {
        pageType: MyLearning.pageType,
        pathTopic: pathTopic,
        path: path
      });
      return false;
    }
    MyLearning._logDebug('ExtraFeaturesEnabled -> true: all checks passed', {
      path: path,
      pathTopic: pathTopic,
      pageType: MyLearning.pageType
    });
    ...
    console.error('MyLearning.extraFeaturesEnabled -> false: error occurred', {
      error: e
    });
    return false;
  }
...
(function () {
  try {
    MyLearning._logDebug('Rushed initialization of extra features');
    // Use unified function to check if menu should be enabled
    if (MyLearning.extraFeaturesEnabled({
      skipMainElementCheck: true
    })) {
      ...
      try {
        ...
        console.error('MyLearning.extraFeaturesEnabled -> error (1): ', e);
      }

      // Inject CSS IMMEDIATELY in head to prevent flash
      if (!document.getElementById('rhs-menu-immediate-style')) {
        MyLearning._logDebug('RhsMenuImmediateStyle -> injecting rhs-menu-immediate-style');
        var style = document.createElement('style');
        style.id = 'rhs-menu-immediate-style';
        style.textContent = "\n          @media (min-width:1206px){\n            /* Apply early width directly when layout is active to avoid initial 100% flash */\n            html.rhs-layout-active #main{width:calc(100% - 270px)!important;max-width:100%;flex:0 0 auto}\n            /* When container switches to two-col flex, keep explicit widths */\n            body.rhs-layout-active .rhs-two-col{display:flex;gap:0;align-items:flex-start}\n            body.rhs-layout-active .rhs-two-col #main{width:calc(100% - 270px)!important;max-width:100%;flex:0 0 auto}\n            body.rhs-layout-active .rhs-two-col #right-side-menu{flex:0 0 270px;display:block;padding:0;margin-left:auto;margin-top:88px}\n          }\n          @media (max-width:1205px){\n            /* Collapse to single column */\n            html.rhs-layout-active #main{width:100%!important}\n            html.rhs-layout-active #right-side-menu{display:none}\n            body.rhs-layout-active .rhs-two-col{display:block}\n            body.rhs-layout-active .rhs-two-col #main{width:100%!important}\n            body.rhs-layout-active .rhs-two-col #right-side-menu{display:none}\n          }\n        ";
    ...
    console.error('MyLearning.extraFeaturesEnabled -> error (2): ', e);
  }
})();
...
MyLearning._apiNameToPathMap = {
  ...
  'api.practice.submit': '/api/classic/set-practice-progress',
  'api.practice.progress': '/api/classic/get-practice-progress',
  'api.practice.bulk_submit': '/api/classic/set-bulk-practice-progress'
};
...
// Centralized fetch for api/user/state — deduplicates concurrent calls so only one
// HTTP request goes out regardless of how many callers invoke this simultaneously.
// Pass cached=true to get the last successful response without a new request.
MyLearning.asyncFetchUserState = function (cached) {
  if (typeof cached === 'undefined') {
    cached = true;
  }
  if (cached && MyLearning._userStateCache !== null) {
    return Promise.resolve(MyLearning._userStateCache);
  }
  if (MyLearning._userStateFetchInFlight !== null) {
    return MyLearning._userStateFetchInFlight;
  }
  MyLearning._userStateFetchInFlight = new Promise(function (resolve) {
    Util.fetch({
      context: 'MyLearning -> fetchUserState',
      method: 'GET',
      url: "".concat(MyLearning._apiBaseUrl, "/api/user/state?lite=true&withBookmarks=true"),
      ...
      callback: function callback(reqRes) {
        MyLearning._userStateCache = reqRes;
        resolve(reqRes);
      }
    });
  });
  MyLearning._userStateFetchInFlight["finally"](function () {
    MyLearning._userStateFetchInFlight = null;
  });
  return MyLearning._userStateFetchInFlight;
};
...
/**
 * When stored exercise answers have fewer comma-separated entries than the live exercise
 * (new questions added), pad with trailing "0" so completion logic matches current length.
 * @param {string} answersStr - stored progress value, e.g. "1,1"
 * @param {number} totalQuestionCount - current number of questions in the exercise
 * @returns {string} padded answers or the original answersStr if unchanged or on error
 */
MyLearning.patchExerciseProgressAnswersIfOutOfSync = function (answersStr, totalQuestionCount) {
  try {
    ...
    var n = Number(totalQuestionCount);
    if (!isFinite(n) || n < 1 || n !== Math.floor(n)) {
      return answersStr;
    }
    var parts = answersStr.split(',');
    if (parts.length >= n) {
      return answersStr;
    }
    ...
    MyLearning._logWarning('patchExerciseProgressAnswersIfOutOfSync -> error', {
      error: e
    });
    return answersStr;
  }
...
MyLearning._handleFinishedPageAuthenticated = function () {
  ...
  var progressAlreadyPushed = false;
  function pushProgressForCurrentPage(prs) {
    MyLearning._logDebug('pushProgressForCurrentPage -> init: ', {
      progressAlreadyPushed: progressAlreadyPushed,
      prs: prs
    });
    if (progressAlreadyPushed) {
      MyLearning._logDebug('pushProgressForCurrentPage -> jumping out because progress already pushed');
      return;
    }
    progressAlreadyPushed = true;
    var x,
      y,
      pos,
      foldername,
  ...
  // If ChallengeEngine is present, gate progress on challenge completion
  try {
    var ce = window.ChallengeEngine;
    if (ce && typeof ce.isChallengeCompleted === 'function' && typeof ce.onChallengeCompleted === 'function') {
      var status = ce.isChallengeCompleted();
      if (status && status.result === true) {
        // Challenge(s) already completed – allow immediate progress push
        pushProgressForCurrentPage({
          context: 'WITH_CHALLENGES -> Immediate'
        });
      } else {
        // Defer progress push until challenges are completed
        ce.onChallengeCompleted('MyLearning:' + (window.location.pathname || ''), function (callbackRes) {
          try {
            if (callbackRes && callbackRes.status && callbackRes.status.result === true) {
              pushProgressForCurrentPage({
                context: 'WITH_CHALLENGES -> Defer'
              });
            }
          } catch (_err) {}
        });
  ...
  // Fallback when ChallengeEngine is not available or usable
  pushProgressForCurrentPage({
    context: 'WITHOUT_CHALLENGES -> Default'
  });
};
...
MyLearning.loadSharedApiModule = function () {
  if (MyLearning._sharedApiModuleLoaded) {
    MyLearning._logDebug('loadSharedApiModule -> skipped, already loaded');
    return;
  }
  MyLearning._sharedApiModuleLoaded = true;

  // Load shared API module
  Util.attachScript({
...
MyLearning._updateConfig = function () {
  var allowedExternalConfigKeys = ['loadSharedApiForAuthenticatedUser'];
  if (typeof MyLearningConfig !== 'undefined') {
    MyLearning._logDebug('MyLearning._updateConfig -> MyLearningConfig: ', MyLearningConfig);
    for (var i = 0; i < allowedExternalConfigKeys.length; i++) {
      var key = allowedExternalConfigKeys[i];
      if (typeof MyLearningConfig[key] !== 'undefined') {
        MyLearning[key] = MyLearningConfig[key];
      }
    }
  }
};
  ...
  Util.objFieldOnSetCallback({
    ...
    callback: function callback(res) {
      ...
      if (res.error.code === '0') {
        ...
        if (UserSession.loggedIn === true) {
          if (MyLearning.loadSharedApiForAuthenticatedUser === true) {
            MyLearning.loadSharedApiModule();
          }
          // find all .nextprev eleents
          var nextPrevBtnsWrapperElm = document.querySelectorAll('.nextprev');
          var sidenavElm = document.getElementById('sidenav');
          if (sidenavElm) {
          ...
          if (MyLearning.extraFeaturesEnabled()) {
            MyLearning.loadSharedApiModule();
            Util.attachCss({
              id: 'my-learning-right-hand-side-menu-style',
              path: "/lib/my-learning/right-hand-side-menu".concat(MY_LEARNING_VERSION ? '.v' + MY_LEARNING_VERSION : '', ".css"),
              callback: function callback(res) {
                MyLearning._logDebug('my-learning-right-hand-side-menu-style -> loaded -> res: ', res);
              }
            });
          ...
          if (nextPrevBtnsWrapperElm.length > 0) {
            MyLearning.loadSharedApiModule();
...
MyLearning.getXpRate = function () {
  ...
  var userInfoCookie = UserSession.getUicCookie();
  if (userInfoCookie) {
    if (typeof userInfoCookie.xpRate !== 'undefined' && userInfoCookie.xpRate) {
      xpRate = parseFloat(userInfoCookie.xpRate);
    } else if (userInfoCookie.plan === 'plus' || userInfoCookie.fa === true) {
      xpRate = 1.5; // boost
    }
  }
...
// options: { topic, problemId }
//   topic     - e.g. 'python' (required)
//   problemId - e.g. 'STRINGS01' (optional); when provided, response includes
//               a 'submissions' array with that problem's attempt history
// Response shape: { pct, completedCodes, submissions? }
//   pct            - float 0-100: % of topic challenges completed (completedCount/totalCount*100)
//   completedCodes - map of { [problemId]: true } for all solved problems in this topic
//   submissions    - (only when problemId given) array of attempt objects in localStorage format
MyLearning.fetchPracticeProgress = function (options, callback) {
  // Backward compat: fetchPracticeProgress(callback)
  if (typeof options === 'function') {
    callback = options;
    options = {};
  }
  var params = [];
  if (options && options.topic) params.push('topic=' + encodeURIComponent(options.topic));
  if (options && options.problemId) params.push('problemId=' + encodeURIComponent(options.problemId));
  var url = MyLearning.getUrl('api.practice.progress') + (params.length ? '?' + params.join('&') : '');
  MyLearning._logDebug('fetchPracticeProgress -> url: ', url);
  Util.fetch({
    context: 'MyLearning -> fetchPracticeProgress',
    method: 'GET',
    url: url,
    callback: callback,
    withUserSession: true,
    handleUserSessionRedirection: false,
    // true: Util.fetch parses JSON and fills res.data; false leaves res.data null (breaks practice UI).
    prepareResponseData: true,
    headers: {
      'X-Cui1': UserSession.getCookie('__c_u_i_1')
    }
...
MyLearning.submitPracticeAttempt = function (attemptData, callback) {
  MyLearning._logDebug('submitPracticeAttempt -> attemptData: ', attemptData);
  if (UserSession.loggedIn !== true) {
    MyLearning._logDebug('submitPracticeAttempt -> skipping: user not logged in');
    return;
  }
  var userInfoCookie = UserSession.getUicCookie();
  if (!userInfoCookie) {
    MyLearning._logError('submitPracticeAttempt -> userInfoCookie not found');
    return;
  }
  var subscriptionPlan = userInfoCookie.plan ? userInfoCookie.plan.toLowerCase() : 'free';
  if (['closed'].includes(subscriptionPlan)) {
    MyLearning._logWarning('submitPracticeAttempt -> skipping: user is on closed plan');
    return;
  }

  // attemptData shape:
  //   problemId       - e.g. 'STRINGS01'
  //   topic           - language, e.g. 'python'
  //   score           - integer 0-100 (raw from Sphere Engine, not capped)
  //   passed          - number of test cases passed (raw from Sphere Engine, not capped)
  //   total           - total test cases (raw from Sphere Engine, not capped)
  //   solutionViewed  - boolean; true means user forfeited XP by viewing the solution
  //   currentUts      - Unix timestamp in seconds at submission time
  //   sourceCode      - user's submitted source code
  //   tests           - first 4 test case result objects (for submissions tab display)
  //   status          - string, e.g. 'Correct', 'Wrong Answer', 'Compilation Error'
  //   yourOutputTest1      - actual output of test 1 (for diff display)
  //   expectedOutputTest1  - expected output of test 1 (for diff display)
  Util.fetch({
    context: 'MyLearning -> submitPracticeAttempt',
    method: 'POST',
    url: MyLearning.getUrl('api.practice.submit'),
...
// Bulk-submit practice attempts migrated from localStorage.
// attempts: array of objects in same shape as submitPracticeAttempt's attemptData.
// Called once on page load when legacy localStorage entries are found.
MyLearning.bulkSubmitPracticeAttempts = function (attempts, callback) {
  MyLearning._logDebug('bulkSubmitPracticeAttempts -> count: ', attempts.length);
  if (UserSession.loggedIn !== true) {
    MyLearning._logDebug('bulkSubmitPracticeAttempts -> skipping: user not logged in');
    return;
  }
  var userInfoCookie = UserSession.getUicCookie();
  if (!userInfoCookie) {
    MyLearning._logError('bulkSubmitPracticeAttempts -> userInfoCookie not found');
    return;
  }
  var subscriptionPlan = userInfoCookie.plan ? userInfoCookie.plan.toLowerCase() : 'free';
  if (['closed'].includes(subscriptionPlan)) {
    MyLearning._logWarning('bulkSubmitPracticeAttempts -> skipping: user is on closed plan');
    return;
  }
  Util.fetch({
    context: 'MyLearning -> bulkSubmitPracticeAttempts',
    method: 'POST',
    url: MyLearning.getUrl('api.practice.bulk_submit'),
    data: JSON.stringify({
      attempts: attempts
    }),
    callback: callback,
